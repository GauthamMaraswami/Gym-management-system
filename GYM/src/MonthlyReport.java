...
public class MonthlyReport implements ActionListener{
	...
	public MonthlyReport() {
		...
		frame1 = new JFrame("Monthly Report");
		view= new JButton("View");
		reset=new JButton("Reset");
				reset.setBounds(50, 200, 150, 30);
		reset.setFont(font1);
		view.setBounds(220, 200,150, 30);
		view.setFont(font1);
		panel1 = new JPanel();
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==reset)
		{
		    try {
	            ...
	            String query="select uid,month(date_of_report),sum(time_spent),sum(calories_burnt) from Daily_report  where month(date_of_report)=month(current_date) group by uid,month(date_of_report)";
	            System.out.println(query);
	            Statement ps=con.createStatement();
	        	ResultSet rs=ps.executeQuery(query);
	        	while(rs.next())
	        	{
	        		 String query2="Select count(*) from Monthly_report  where month=month(current_date) and uid="+rs.getString(1);
	        		...
	        		String query1="Insert into Monthly_report (uid,month,tot_calories) values("+rs.getString(1)+" , "+rs.getString(2)+" , "+rs.getString(4)+")";
		...
		if(e.getSource()==view)
		{
			...
			try {
	            ...
	            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/gym?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC","root","root"  );
	            //Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/gym","root","root");
	            String query="select D.id,U.name,D.uid,D.month,D.tot_calories,D.food_suggestions,I.name,D.rank from Monthly_report as D,User as U,Instructor as I where  D.uid=U.User_id and U.Inst_id=I.Inst_id order by D.month desc,D.rank ";
	            System.out.println(query);
	            Statement ps=con.createStatement();
	        	ResultSet rs=ps.executeQuery(query);
	        	int ptr=50;
